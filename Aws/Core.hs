...
loadCredentialsFromInstanceMetadata = do
    ...
    if not avail
      then return Nothing
        ...
        maybeArn <- liftIO $ E.catch (getInstanceMetadata mgr "latest/meta-data/iam" "security-credentials" >>= return . Just) (\(_ :: HTTP.HttpException) -> return Nothing)
        case maybeArn of
          Just arn ->
            do
              creds <- liftIO $ E.catch (getInstanceMetadata mgr "latest/meta-data/iam/security-credentials" (BU.toString . L.toStrict $ arn) >>= return . Just) (\(_ :: HTTP.HttpException) -> return Nothing)
